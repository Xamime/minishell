...
/*   Updated: 2023/05/26 14:41:14 by mdesrose         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_redirect(t_redir *redirs, char *str)
{
	...
	*fd = 0;
	if (*str == '>' && *(str + 1) != '>')
		fd = open_list("outfile", fd, filename, redirs);
	else if (*str == '>' && *(str + 1) == '>')
		fd = open_list("append", fd, filename, redirs);
	else if (*str == '<' && *(str + 1) != '<')
		fd = open_list("infile", fd, filename, redirs);
	free(filename);
	if (*fd == 0)
	{
		free(fd);
